...
public class GUIVisualization extends JFrame {
	...
	public GUIVisualization(String plotType) {
		...
		for (; i <= sampleSize; i++) {
			...
			for (int j = 0; j < division; j++) {
				manager.searchStock("Sym" + (i - division + j));
			}
			long endTime = System.nanoTime();
			dataPointsX.add(i / division);
			dataPointsY.add((endTime - startTime) / 1_000);
		}
	...
	drawGraph(Graphics g) {
		...
		if (plotType.equals("line")) {
			for (int i = 0; i < dataPointsX.size() - 1; i++) {
				int x1 = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding +
					 labelPadding;
				int y1 = afterPaddingY - (int)((dataPointsY.get(i) * 1.0) / maxYValue *
							       (height - padding * 2 - labelPadding));
				int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) +
					 padding + labelPadding;
				int y2 = afterPaddingY - (int)((dataPointsY.get(i + 1) * 1.0) / maxYValue *
							       (height - padding * 2 - labelPadding));
				g2.drawLine(x1, y1, x2, y2);
			}
		} else if (plotType.equals("scatter")) {
			for (int i = 0; i < dataPointsX.size(); i++) {
				int x = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding +
					labelPadding;
				int y_val = (int)(dataPointsY.get(i) * multiplier);
				if (y_val > maxYValue)
					continue;
	...
	getMaxYValue() { /*
      ...
      if (maxList.size() < count) {
        maxList.add(dataPointsY.get(i));
			} else {
        for (int j = 0; j < count; j++) {
          if (dataPointsY.get(i) > maxList.get(j)) {
            maxList.set(j, dataPointsY.get(i));
						break;
					}
				}
			}
    ...
    sum += maxList.get(i);
		return sum / count;
    */
		return dataPointsY.stream().max(Long::compare).get().longValue();
	}
	public static void
	main(String[] args) {
